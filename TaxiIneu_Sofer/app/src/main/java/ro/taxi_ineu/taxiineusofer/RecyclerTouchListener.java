...
public class RecyclerTouchListener implements RecyclerView.OnItemTouchListener, OnActivityTouchListener {
    ...
    private int bgWidth = 1, bgWidthLeft = 1, fgWidth = 1;// 1 and not 0 to prevent dividing by zero
    ...
    //by cmf
    private boolean swiped_dreapta=false,swiped_stanga=false;
    private OnSwipeListener2 swipeableListenerDreapta;
...
//        mSwipeListener = listener;
    ...
    //by cmf
    public RecyclerTouchListener setSwipeableDreapta(OnSwipeListener2 listener) {
        this.swipeableListenerDreapta=listener;
        return this;
    }
    ...
    //void onSwipeDreapta(int position);
    ...
    public void openSwipeOptions(int position) {
        ...
        //by cmf
        if (fgWidth < 2) {
            if (act.findViewById(fgViewID) != null)
                fgWidth = act.findViewById(fgViewID).getWidth();
            //heightOutsideRView = screenHeight - rView.getHeight();
        }
...
//            bgWidth = rView.getWidth();
        ...
        //by cmf
        if (swipeable && fgWidth < 2) {
//            fgWidth = rView.getWidth();
            if (act.findViewById(fgViewID) != null)
                fgWidth = act.findViewById(fgViewID).getWidth();
...
//                        bgView.getLayoutParams().height = fgView.getHeight();
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (swiped_dreapta){
                    ...
                    swipeableListenerDreapta.onSwipeDreapta(touchedPosition);
                }
                swiped_dreapta=false;
                swiped_stanga=false;
                //end by cmf
                /*
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (swipeable && isFgSwiping && !unSwipeableRows.contains(touchedPosition)) {
                    ...
                    //by cmf
                    if (deltaX > touchSlop && !bgVisible && !swiped_stanga) {
                        //swiped_dreapta=false,swiped_stanga=false;

                        float translateAmount2 = deltaX - mSwipingSlop;
                        //int fgWidth2=fgWidth-bgWidth;
                        int fgWidth2=fgWidth/2;
                        //System.out.println("right="+translateAmount2+"  "+fgWidth);
//                        if ((Math.abs(translateAmount) > bgWidth ? -bgWidth : translateAmount) <= 0) {
                        ...
                        fgView.setTranslationX(Math.abs(translateAmount2) > fgWidth2 ? -fgWidth2 : translateAmount2);
                        //fgView.setTranslationX(Math.abs(translateAmount2));
                        if (fgView.getTranslationX() <0) {
                            fgView.setTranslationX(fgWidth2);
                            swiped_dreapta=true;
                            //System.out.println("dreapta="+swiped_dreapta);
                        }
//                        }
                        ...
                        if (fadeViews != null) {
                            for (int viewID : fadeViews) {
                                touchedView.findViewById(viewID).setAlpha(1- (Math.abs(translateAmount2) / bgWidth));
                            }
                        }
                        //end by cmf
...
//                        }
    ...
    //by cmf
    public interface OnSwipeListener2 {
        void onSwipeDreapta(int position);
